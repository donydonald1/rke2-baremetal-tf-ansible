...
async def add_retention_policy(client, project_id):
    ...
    await client.create_retention_policy(
        RetentionPolicy(
            ...
            scope=RetentionPolicyScope(level="project", ref=project_id),
        )
    )
...
async def process_policy_in_projects(client):
    ...
    for project in projects:
        ...
        if not retention_id:
            print(
                f"No retention policy found for project: id {project.project_id} name {project.name!r} "
            )
            await add_retention_policy(client, project.project_id)
        else:
            await update_retention_policy(client, retention_id, project.project_id)
            print(
                f"Retention policy found for project: id {project.project_id} name {project.name!r}"
            )
...
async def main():
    parser = argparse.ArgumentParser(
        description="Manage retention policies and projects in Harbor"
    )
    parser.add_argument(
        "--url", required=True, help='Harbor URL, e.g. "https://registry.com/api/v2.0"'
    )
    parser.add_argument(
        "--username", required=True, help='Harbor username, e.g. "admin"'
    )
